...
bool MixedFlowControlPublisher::init()
{
    ...
    // flow_control_300k_per_sec->max_bytes_per_period = 30 * 1000;
...
void MixedFlowControlPublisher::run()
{
    ...
    */
	...
	int a_send_sum = 600, b_send_sum = 300, c_send_sum = 200, d_send_sum = 100;
        ...
        short priority, int send_sum, std::string type_name, int ms_per_msg, int* thread_end)
{
	...
	statisticsOut << "msg_index, msg_priority, msg_type, msg_start_time(ts), msg_fail_send" << std::endl;
    MixedMsg st;
    Time_t now_time;
    ...
    while (index < send_sum)
    {
        ...
        if ((*writer)->write(&st))
        {
            st.is_overtime(false); // 用未超时来表示发送成功
        }
        else
        {
            st.is_overtime(true);
